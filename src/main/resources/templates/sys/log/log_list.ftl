...
<script>
	layui.use(["element", "jquery", "table", "form", "crud", "laydate"], function () {
		...
		var layTable = table.render({
			...
			size: "sm",
			cols: [[
				{field: "LOG_ID", type: "checkbox", fixed: "left"},
				{title: "NO.", type: "numbers"},
				{field: "USERNAME", title: "用户名"},
				{field: "CONTENT", title: "事件"},
				{field: "CZTIME", title: "操作时间"},
		...
		table.on("toolbar(tableFilter)", function (obj) {
			switch (obj.event) {
				case "batchDel":
					crud.batchDel('/log/deleteAll', table, "LOG_ID");
					break;
			}
		});
		...
		table.on("tool(tableFilter)", function (obj) {
			...
			switch (obj.event) {
				case "del":
					crud.imDel('/log/delete', data.LOG_ID);
					break;
			}
		});
	})
