	...
	layui.use(["element", "jquery", "table", "form", "crud", "laydate"], function () {
		...
		var layTable = table.render({
			...
			cols: [[
				...
				{field: "ASSIGNEE_", title: "办理人or角色"},
				{field: "NAME_", title: "任务节点"},
				{field: "START_TIME_", title: "开始时间", width: 150},
				{field: "END_TIME_", title: "办完时间", width: 150},
				{field: "ZTIME", title: "用时", width: 150},
		...
		table.on("toolbar(tableFilter)", function (obj) {
			switch (obj.event) {
				case "batchDel":
					crud.batchDel('/hitask/deleteAll', table, "USER_ID");
					break;
				case "add":
					crud.imAdd('/im/act/hitask/ruprocdef_edit', layTable, '50%', '70%');
					break;
			}
		});
		...
		table.on("tool(tableFilter)", function (obj) {
			...
			switch (obj.event) {
				case "edit":
					...
					break;
				case "del":
					crud.imDel('/hitask/delete', data.ID_);
					break;
			}
		});
	})
