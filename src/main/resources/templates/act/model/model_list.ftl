	...
	layui.use(["element", "jquery", "table", "form", "crud", "laydate"], function () {
		...
		var layTable = table.render({
			...
			cols: [[
				...
				{field: "NAME_", title: "模型名称"},
				{field: "DNAME", title: "分类"},
				{field: "CREATE_TIME_", title: "创建时间", width: 150},
				{field: "LAST_UPDATE_TIME_", title: "最后更新时间", width: 150},
		...
		table.on("toolbar(tableFilter)", function (obj) {
			switch (obj.event) {
				case "batchDel":
					...
					break;
				case "add":
					crud.imAdd('/im/act/model/model_edit', layTable, '50%', '70%');
					break;
			}
		});
		...
		table.on("tool(tableFilter)", function (obj) {
			...
			switch (obj.event) {
				case "edit":
					crud.imEdit('/im/act/model/model_edit', data, layTable, '50%', '70%');
					break;
				case "del":
					crud.imDel('/user/delete', data.ID_);
					break;
			}
		});
	})
