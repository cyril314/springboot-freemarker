	...
	layui.use(["element", "jquery", "table", "form", "crud", "laydate"], function () {
		...
		var layTable = table.render({
			...
			cols: [[
				...
				{field: "ASSIGNEE_", title: "当前节点(待办人)"},
				{field: "NAME_", title: "当前任务"},
				{field: "CREATE_TIME_", title: "创建时间", width: 150},
				{field: "SUSPENSION_STATE_", title: "操作", align: "center", templet: "#statusTpl"}
			]],
		...
		table.on("toolbar(tableFilter)", function (obj) {
			switch (obj.event) {
				case "batchDel":
					crud.batchDel('/rutask/deleteAll', table, "USER_ID");
					break;
				case "add":
					crud.imAdd('/im/act/rutask/rutask_edit', layTable, '50%', '70%');
					break;
			}
		});
		...
		table.on("tool(tableFilter)", function (obj) {
			...
			switch (obj.event) {
				case "edit":
					crud.imEdit('/im/act/rutask/rutask_edit', data, layTable, '50%', '70%');
					break;
				case "del":
					crud.imDel('/rutask/delete', data.ID_);
					break;
			}
		});
	})
...
<script type="text/html" id="statusTpl">
	{{#  if(d.SUSPENSION_STATE_ == '1'){ }}
	<a href="javascript:" title="委派" lay-event="edit" class="layui-btn layui-btn-normal layui-btn-xs">委派</a>
	<a href="javascript:" title="办理" lay-event="del" class="layui-btn layui-btn-normal layui-btn-xs">办理</a>
	{{#  } else { }}
	<span class="layui-btn layui-btn-warm layui-btn-xs">已挂起</span>
	{{#  } }}
</script>
</body>
</html>
