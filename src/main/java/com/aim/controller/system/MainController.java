...
public class MainController extends IController {
	...
	public String page(HttpServletRequest request) {
		...
		String page = uri.replaceFirst("/im", "");
		logger.info("当前跳转的页面: {}", page);
		return page;
	}
	...
	public Object getMenu() throws Exception {
		List<TreeData> mtList = new ArrayList<TreeData>();
		...
		if (user != null) {
			...
			List<Menu> menuList = this.getAttributeMenu(session, USERNAME, roleRights, getArrayRoleRights(ROLE_IDS), this.getUQX(user, role, shiroSet), this.getUQX2(user), shiroSet);        //菜单缓存以及赋值权限
			if (null == session.getAttribute(USERNAME + Const.SHIROSET)) {
				session.setAttribute(USERNAME + Const.SHIROSET, shiroSet);                //存放菜单权限标识
			}

			if (menuList.size() > 0) {
				convertMenu(mtList, menuList);
			}
		}
		return mtList;
	}
	...
	public List<Menu> getAttributeMenu(Session session, String USERNAME, String roleRights, List<String> arrayRoleRights, Map<String, String> uqxmap, Map<String, List<String>> uqxmap2, Collection<String> shiroSet) throws Exception {
		...
		if (null == session.getAttribute(USERNAME + Const.SESSION_ALLMENU)) {
			...
			if (Tools.notEmpty(roleRights)) {
				//根据角色权限获取本权限的菜单列表
				allmenuList = this.readMenu(USERNAME, allmenuList, roleRights, arrayRoleRights, shiroSet, uqxmap, uqxmap2);
			}
	...
	public Object logout() throws Exception {
		...
		subject.logout();
		logService.save(USERNAME, "退出系统"); //记录日志
