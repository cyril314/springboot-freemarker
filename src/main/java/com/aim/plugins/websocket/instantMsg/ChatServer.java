package com.aim.plugins.websocket.instantMsg;

import com.aim.entity.PageData;
import com.aim.service.im.*;
import com.aim.util.Const;
import com.aim.util.DateUtil;
import com.aim.util.SpringUtil;
...
public class ChatServer extends WebSocketServer {
    ...
    public void usersendmsg(String message, WebSocket conn) {
        ...
        if ("friend".equals(type)) {
            result.put("id", user);        //此id得是发送者的了(消息来源)
    ...
    public void userjoin(String user, WebSocket conn) {
        ...
        try {
            ...
            if (null == ipd || "online".equals(ipd.getString("ONLINE"))) {                    //只有用户设置的状态为在线的时候，才会向别人发送上线消息
                ...
                for (int i = 0; i < varlist.size(); i++) {
                    if (i == 0) result.put("name", varlist.get(i).getString("NAME"));
                    ...
                    if (null != mws)
                        ChatServerPool.mobilesendMessageToUser(mws, result.toString());            //只向有自己好友的用户发送上线消息;
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
            ...
            try {
                ...
                if (null == ipd || "online".equals(ipd.getString("ONLINE"))) {                        //只有用户设置的状态为在线的时候，才会向别人发送上线消息
                    ...
                    for (int i = 0; i < varlist.size(); i++) {
                        ...
                        WebSocket mws = ChatServerPool.mobilegetWebSocketByUser(varlist.get(i).getString("USERNAME"));
                        if (null != mws)
                            ChatServerPool.mobilesendMessageToUser(mws, result.toString());//只向有自己好友的用户发送下线消息
                    }
                }
            } catch (Exception e) {
                e.printStackTrace();
    ...
    public void useronline(String user, WebSocket conn) {
        ...
        try {
            ...
            for (int i = 0; i < varlist.size(); i++) {
                ...
                WebSocket mws = ChatServerPool.mobilegetWebSocketByUser(varlist.get(i).getString("USERNAME"));
                if (null != mws)
                    ChatServerPool.mobilesendMessageToUser(mws, result.toString());                //只向有自己好友的用户发送在线状态消息（手机端）
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void userhide(String user, WebSocket conn) {
        ...
        try {
            ...
            for (int i = 0; i < varlist.size(); i++) {
                ...
                WebSocket mws = ChatServerPool.mobilegetWebSocketByUser(varlist.get(i).getString("USERNAME"));
                if (null != mws)
                    ChatServerPool.mobilesendMessageToUser(mws, result.toString());                //只向有自己好友的用户发送隐身状态消息（手机端）
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    public void applyfriend(String message, WebSocket conn) {
        JSONObject result = new JSONObject();
        result.put("type", "user_applyfriend");
    ...
    public void userremove(String message, WebSocket conn) {
        String arrmessage[] = message.split(",fh,");
        JSONObject result = new JSONObject();
        result.put("type", "user_emove");
        result.put("user", arrmessage[0]);
    ...
    public void agreefriend(String message, WebSocket conn) {
        ...
        try {
            ...
            JSONObject result = new JSONObject();
            result.put("username", pd.getString("NAME"));                                                            //当前用户姓名
            result.put("id", USERNAME);                                                                                //当前用户名
            result.put("groupid", pd.getString("FGROUP_ID"));                                                        //分组ID
            result.put("status", null == pd.get("ONLINE") ? "online" : pd.getString("ONLINE"));                        //在线状态
            result.put("sign", null == pd.get("AUTOGRAPH") ? "个性签名" : pd.getString("AUTOGRAPH"));                    //个性签名
            result.put("avatar", null == pd.get("PHOTO2") ? "assets/images/user/avatar-2.jpg" : pd.getString("PHOTO2"));            //当前用户头像
            result.put("type", "agreefriend");
    ...
    public void agreeqgroup(String message, WebSocket conn) {
        ...
        try {
            pd = qgroupService.findById(pd);
            JSONObject result = new JSONObject();
            result.put("type", "agreeqgroup");
            result.put("id", QGROUP_ID);                        //群ID
            result.put("groupname", pd.getString("NAME"));        //群名
            result.put("avatar", (null == pd.getString("PHOTO") || "".equals(pd.getString("PHOTO"))) ? "assets/images/user/avatar-2.jpg" : pd.getString("PHOTO"));        //群头像
    ...
    public void delqgroup(String QGROUP_ID, WebSocket conn) {
        ...
        try {
            JSONObject result = new JSONObject();
            result.put("type", "delqgroup");
            result.put("QGROUP_ID", QGROUP_ID);
